...
// O histórico começará vazio e será preenchido pelo Supabase
let history = [];
...
// ======================================================
// CONVERTE HORÁRIO DO SUPABASE (UTC) PARA HORA DO BRASIL
// ======================================================
...
async function fetchHistoryFromSupabase() {
    try {
        ...
        if (resposta.ok) {
            ...
            // Mapeia os dados do Supabase para o formato do seu site
            history = dados.map(item => ({
                plant: "PlantCare ESP32",
                icon: item.mensagem.includes("REG") ? "💧" : "📡",
                time: formatarDataBrasil(item['horários']),
                value: item.mensagem
            }));

            // Atualiza a tela com o novo histórico
            render();
        } else {
            console.error("Erro ao carregar histórico do Supabase:", resposta.status);
        }
    } catch (erro) {
        console.error("Erro de conexão ao buscar histórico:", erro);
    }
}
...
function card(p) {
    ...
    <article class="card" onclick="selectPlant(${p.id})">
        ...
        <div class="percent">${p.humidity}%</div>
...
function render() {
    ...
    document.getElementById("plantsList").innerHTML =
        ...
        <div class="plant-item">
            <div class="rowleft">
                ...
                <div>
                    <strong>${p.name}</strong>
                    <p class="muted">${p.humidity}% de umidade · ${p.status}</p>
                </div>
            </div>
            <button onclick="selectPlant(${p.id})">Detalhes</button>
        </div>
        `).join("");
    ...
    const hist =
        history.length > 0
            ? history.map(h => `
              ...
              `).join("")
            : `<p class="muted" style="padding: 10px;">Carregando histórico do Supabase...</p>`;
    ...
    document.getElementById("plantSelect").innerHTML =
        plants.map(p => `<option>${p.name}</option>`).join("");
...
function updateMain() {
    ...
    document.getElementById("mainHumidity").textContent = pct + "%";
    document.getElementById("mainRing").style.background =
        `conic-gradient(var(--g) 0 ${pct}%, #d5dfd0 ${pct}% 100%)`;
    ...
    try {
        const resposta = await fetch(
            `${supabaseUrl}/rest/v1/comandos`,
            {
                ...
                body: JSON.stringify({ mensagem: "REGAR" })
            }
        );

        if (resposta.ok) {
            toast("💧 Comando de rega enviado!");
            // Atualiza o histórico 2 segundos após enviar o comando
            setTimeout(fetchHistoryFromSupabase, 2000);
        } else {
            console.log("Erro Supabase:", resposta.status, await resposta.text());
...
document.getElementById("waterBtn").onclick = waterNow;
...
document.getElementById("sensorInterval").onclick = () => {
    toast("⏱️ Intervalos disponíveis: 1, 5, 10 ou 15 minutos.");
};

// Inicializa a renderização e busca os dados no Supabase
render();
fetchHistoryFromSupabase();

// Atualiza o histórico do banco automaticamente a cada 10 segundos
